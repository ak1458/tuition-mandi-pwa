...
/**
 * Standalone brand mark — open book (learning) + spark. `variant='gold'` draws a
 * marigold book (use on dark/ink surfaces); `variant='ink'` draws an ink book.
 * Mirrors the design system's TagMark.
 */
export function BrandMark({ size = 36, variant = 'gold' }: { size?: number; variant?: 'gold' | 'ink' }) {
  const main = variant === 'ink' ? 'var(--ink)' : 'var(--marigold)'
  const detail = variant === 'ink' ? 'var(--marigold)' : 'var(--on-marigold)'
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" style={{ flexShrink: 0 }} aria-hidden="true">
      <path d="M24 14.5 C 19 11.2, 12 10.4, 6.5 12.2 V 35.4 C 12 33.6, 19 34.2, 24 37.6 Z" fill={main} />
      <path d="M24 14.5 C 29 11.2, 36 10.4, 41.5 12.2 V 35.4 C 36 33.6, 29 34.2, 24 37.6 Z" fill={main} />
      <path d="M24 15.4 V 36.6" stroke={detail} strokeWidth="1.7" strokeLinecap="round" />
      <g stroke={detail} strokeWidth="1.6" strokeLinecap="round" opacity="0.5">
        <path d="M11 19 H 19.5 M11 23.5 H 19.5 M11 28 H 18" />
        <path d="M28.5 19 H 37 M28.5 23.5 H 37 M30 28 H 37" />
      </g>
      <path d="M38.5 6 l1.05 2.45 2.45 1.05 -2.45 1.05 -1.05 2.45 -1.05 -2.45 -2.45 -1.05 2.45 -1.05 Z" fill={detail} />
    </svg>
  )
}
