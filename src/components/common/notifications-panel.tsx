...
const TONE_BY_TYPE: Record<NotificationType, string> = {
  inquiry: 'bg-[#f1edff] text-[#d6850a]',
  fee: 'bg-[#fff4df] text-[#c87b22]',
  attendance: 'bg-[#dcf1e7] text-[#138a5e]',
  report: 'bg-[#fbe6e1] text-[#e14b36]',
  system: 'bg-[#f4f1ea] text-[#847a6c]',
}
  ...
  return (
    <article
      className={cx(
        ...
        notification.is_read ? 'border-[#e5decf] bg-white' : 'border-[#fcefd2] bg-[#fcefd2]',
      )}
    >
      <button
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13px] font-black text-[#1c1916]">{notification.title}</p>
          <p className="mt-0.5 line-clamp-2 text-[11px] font-semibold leading-4 text-[#5d544c]">{notification.body}</p>
          <p className="mt-1 text-[10px] font-bold text-[#847a6c]">{timeAgo(notification.created_at)} ago</p>
        </div>
      </button>
      <button
        aria-label="Dismiss"
        className="grid h-7 w-7 place-items-center rounded-full text-[#847a6c] hover:bg-[#f4f1ea]"
        onClick={(event) => {
...
export function NotificationsPanel({ userId, open, onClose }: NotificationsPanelProps) {
  ...
  return (
    <div aria-modal className="fixed inset-0 z-[60]" role="dialog">
      ...
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-[420px] flex-col bg-[#f4f1ea] shadow-[-12px_0_32px_rgba(53,38,22,0.18)]">
        <header className="flex items-center justify-between border-b border-[#efe4d6] bg-white px-4 py-3">
          <div>
            <h2 className="text-base font-black text-[#1c1916]">Notifications</h2>
          ...
          <div className="flex items-center gap-2">
            {notifications.length > 0 && unreadCount > 0 && (
              <button
                className="rounded-lg border border-[#e5decf] bg-white px-3 py-1.5 text-[11px] font-black text-[#d6850a]"
                onClick={markAllRead}
                type="button"
              >
                Mark all read
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          {notifications.length === 0 ? (
            <div className="rounded-[20px] border border-[#e5decf] bg-white p-8 text-center shadow-sm">
              <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#f4f1ea] text-[#847a6c]">
                <Icon className="h-7 w-7" name="bell" />
              </div>
              <p className="mt-4 text-sm font-black text-[#1c1916]">Koi notification nahi hai</p>
              <p className="mt-1 text-xs font-semibold text-[#847a6c]">
